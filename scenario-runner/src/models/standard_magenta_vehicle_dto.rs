/*
 * OpenAPI definition
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// StandardMagentaVehicleDto : The vehicle data transfer object
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct StandardMagentaVehicleDto {
    #[serde(rename = "activeTime")]
    pub active_time: i32,
    #[serde(rename = "coordX")]
    pub coord_x: f32,
    #[serde(rename = "coordY")]
    pub coord_y: f32,
    #[serde(rename = "customerId")]
    pub customer_id: uuid::Uuid,
    #[serde(rename = "distanceTravelled")]
    pub distance_travelled: f32,
    #[serde(rename = "id")]
    pub id: uuid::Uuid,
    #[serde(rename = "isAvailable")]
    pub is_available: bool,
    #[serde(rename = "numberOfTrips")]
    pub number_of_trips: i32,
    #[serde(rename = "remainingTravelTime")]
    pub remaining_travel_time: i32,
    #[serde(rename = "vehicleSpeed")]
    pub vehicle_speed: f32,
}

impl StandardMagentaVehicleDto {
    /// The vehicle data transfer object
    pub fn new(active_time: i32, coord_x: f32, coord_y: f32, customer_id: uuid::Uuid, distance_travelled: f32, id: uuid::Uuid, is_available: bool, number_of_trips: i32, remaining_travel_time: i32, vehicle_speed: f32) -> StandardMagentaVehicleDto {
        StandardMagentaVehicleDto {
            active_time,
            coord_x,
            coord_y,
            customer_id,
            distance_travelled,
            id,
            is_available,
            number_of_trips,
            remaining_travel_time,
            vehicle_speed,
        }
    }
}

